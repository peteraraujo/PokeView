...
import {capitalizeFirstLetter, formatPokemonHeight, formatPokemonWeight} from "../../../utils/formatters.ts";
...
export const useGetPokemonDetails = (name: string) => {
    return useQuery({
        ...
        select: (data): FormattedPokemonDetails => ({
            ...data,
            displayWeight: formatPokemonWeight(data.weight),
            displayHeight: formatPokemonHeight(data.height),
            formattedName: capitalizeFirstLetter(data.name),
        }),
    });
};
